...
| TD-15 ✅ | **Carga de app = 12 requests** (sin `getBootstrap`) | GAS-C2 | Latencia alta y ×12 invocaciones por carga/refresh | M | **HECHO** (`98f8c19`): acción `getBootstrap` (`Reports.gs`) lee las 12 hojas en una ejecución; frontend `pullData()` la usa con fallback a `pullAll`. ⚠️ Requiere desplegar el backend. |
| TD-16 | **`SpreadsheetApp.openById` sin cachear** (5–8/req) | GAS-I1 | Aperturas repetidas caras por request | S | Memoizar handle del spreadsheet por ejecución. **Quick win.** |
