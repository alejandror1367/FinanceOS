...
| TD-39 ✅ | Recurrentes sin ejecución automática | F-16 / SessionState | **HECHO** (`d37a938`): `recurringService.runDueRecurring()` al cargar la app materializa los recurrentes vencidos (catch-up con tope) como tx con id determinista `rec_{id}_{fecha}` (idempotente FE/BE) y avanza `nextRunDate`. Client-side, sin deploy. +13 tests. |
...
| ID | Problema | Origen | Impacto | Esf | Estado |
|----|----------|--------|---------|-----|--------|
| TD-54 ✅ | **Transacciones en divisa extranjera suman 1:1** en `monthlyIncome/Expense`, `cashflow` y presupuestos — convertir bien exige tasa histórica a la fecha de la tx, no la actual | Sprint A residual (FIN-005) | Flujo de caja/presupuestos distorsionados si hay tx en USD/EUR | M | **HECHO** (`0e5841f`, desplegado 2026-06-10): `transactionAmountBase()` usa `amountBase` persistido o `fxRateToBase` histórico; si falta, excluye la tx y `fxGaps()` la reporta aunque exista tasa spot. Aplicado a `monthlyIncome/Expense`, `cashflow`, `budgetConsumed`, `categorySpend`, `categoryTrends`, `expenseByCategory`, `topCategoryChange` y `Reports.gs`. Tests: 168/168 selectors. Deploy ✅: `Config.gs`/`Transactions.gs`/`Reports.gs` + `setupDatabase()` ejecutado (columnas `amountBase`, `fxRateToBase`, `fxRateDate`). Escritor: `dataService.create` sella la tasa spot al crear la tx. |
